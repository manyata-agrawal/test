 ...
 */
public class ConnectorControllerPreparedst extends Controller implements org.quartz.Job {

    @Override
    public /*synchronized*/ void execute(JobExecutionContext context) {
        try {
            Logger.info("Inside execute() implementation DoCopyController " + new Date());
            ...
            Logger.info("Out of execute() implementation  DoCopyController" + new Date());
        } catch (Exception e) {
    ...
    public /*synchronized*/ void connectorLogic(SqlRow sqlRow, Long connectorId) {
        ...
        try {
            ...
            if (sqlRow.get("ingestion_type_id") != null && sqlRow.getInteger("ingestion_type_id") == 1) {
                ...
                String targetTableQuery = "select * from bodhee.jdbc_target_table where  id = " + sqlRow.getInteger("target_table_id") + "";
                ...
                if (targetUniqueField != null && sourceUniqueField != null && !targetUniqueField.equalsIgnoreCase("")
                    ...
                    if (!sqlRow.getBoolean("history_load")) {
                        sourceDataSql.append(" and ").append(sourceUniqueField).append(" >  ? ");
                        sourceDataSql.append(" order by ").append(sourceUniqueField).append(" asc ");
                        if (sourceDriver.contains("postgresql")) {
                        ...
                        if (currValData != null && currValData.get("current_unique_value") != null) {
                            currVal = currValData.get("current_unique_value") == null ? 0 : currValData.get("current_unique_value");
                            Logger.info("No unique value found");
                            dataFromSource = Ebean.getServer(sourceServer).createSqlQuery(sourceDataSql.toString()).setParameter(1, currVal).findList();
                            functionToCopy(dataFromSource, sqlRow, connectorId, targetCol, targetUniqueField, false, target);
                        ...
                        String andClause = appendAndClause(historyCondition);
                        ...
                        if (!CollectionUtils.isEmpty(dataFromSource)) {
                            functionToCopy(dataFromSource, sqlRow, connectorId, targetCol, targetUniqueField, true, target);
    ...
    public /*synchronized*/ void functionToCopy(List<SqlRow> dataFromSource, SqlRow sqlRow, Long connectorId, StringBuffer targetCol, String targetUniqueField, Boolean histFlag, String target) throws SQLException {
        Transaction transaction = null;
        transaction = Ebean.beginTransaction();
        Connection connection = transaction.getConnection();
        //connection.setAutoCommit(false);
        //Statement stmt = connection.createStatement();
        PreparedStatement stmt = connection.prepareStatement("INSERT INTO " + target + " ( " + targetCol + " )" + "  " + "(select  " + targetCol + " from  jsonb_populate_record(null:: " + target + " " + ", ?  ) )");
            ...
            if (!CollectionUtils.isEmpty(dataFromSource)) {
                ...
                if (dataFromSource.get(0) != null && dataFromSource.get(0).get(targetUniqueField) != null) {
                    ...
                    for (JsonNode jNode : Json.parse(res.replaceAll("'", ""))) {
                        ...
                        if (targetCol != null && target != null && jNode != null && jNode.size() > 0 && !targetCol.toString().equalsIgnoreCase("") && !target.equalsIgnoreCase("")) {

                            stmt.setObject(1, jNode.toString());
    ...
    public String appendAndClause(JsonNode historyCondition) {
        StringBuffer sourceDataSql = new StringBuffer();
        if (historyCondition != null) {
            ...
            for (JsonNode jNode : historyCondition) {
                ...
                } else if (jNode.get("datatype") != null && jNode.get("value") != null && (jNode.get("datatype").asText().equalsIgnoreCase("string") || jNode.get("datatype").asText().equalsIgnoreCase("date"))) {
